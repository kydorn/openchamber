...
const capitalizeFirst = (value: string | null | undefined): string | null => {
...
const fetchNeuralwattQuota = async (): Promise<ProviderResult> => {
  ...
  try {
    ...
    if (subscription) {
      ...
      const windowSeconds = neuralwattWindowSeconds(billingInterval);
      if (subLabel) {
    ...
    if (allowance) {
      ...
      const labelParts: string[] = labelName ? [`${capitalizeFirst(labelName)} key`] : [];
